...
export default function ShareFuel({ carMake, stationName, price, fuelLabel = 'fuel', postcode }) {
  ...
  const handleNativeShare = async () => {
    try { await navigator.share({ title: 'FuelAlerts', text: message, url }) } catch {}
  }
  ...
  return (
    <div className={styles.shareWrap}>
      ...
      <div className={styles.shareBtns}>
        {hasNative && (
          <button className={`${styles.shareBtn} ${styles.shareBtnNative}`} onClick={handleNativeShare}>
            <ShareIcon /> Share
          </button>
        )}
        <a className={`${styles.shareBtn} ${styles.shareBtnWA}`} href={`https://wa.me/?text=${enc}`} target="_blank" rel="noopener noreferrer">
          <WAIcon /> WhatsApp
        </a>
        <a className={`${styles.shareBtn} ${styles.shareBtnSMS}`} href={`sms:?body=${enc}`}>
          <MsgIcon /> iMessage
        </a>
        <a className={`${styles.shareBtn} ${styles.shareBtnFB}`} href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`} target="_blank" rel="noopener noreferrer">
          <FBIcon /> Facebook
        </a>
        {!hasNative && (
          <button className={`${styles.shareBtn} ${styles.shareBtnCopy}`} onClick={handleCopy}>
            {copied ? '✓ Copied!' : '📋 Copy'}
          </button>
        )}
      </div>
...
const ShareIcon = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
...
const MsgIcon = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
